...
export default function Services({ isServices }: { isServices: boolean }) {
    return (
        <section className="w-full max-w-[1197px] px-4 sm:px-6 md:px-8 lg:px-0 min-h-screen py-12 md:py-16 lg:py-[80px] mx-auto flex flex-col items-center gap-8 sm:gap-12 lg:gap-[64px]">
            ...
            <section className="w-full h-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-[24px]">
                {ServicesData.map((item, index) => (
                    <div key={index} className='w-full sm:w-auto h-auto shadow-lg rounded-[10px]'>
                        <Image
                            src={item.image}
                            alt='services'
                            width={281}
