...
export default function Profile() {
    ...
    const getAdvertiserData = async () => {
      try {
        ...
        setAdvertiser({
          ...
          targetAudience: advertiserData?.targetAudience.map(
            (item: TargetAudience) => item.value
          ),
  ...
  if (!Advertisercampaigns || !advertiser)
    return <p className="text-crimson-500">No Data found</p>;

  const activeCampaigns = Advertisercampaigns.filter(
    (item) => item.status === "active"
  );
  const scheduledCampaigns = Advertisercampaigns.filter(
    (item) => item?.status?.trim() === "scheduled"
  );
  const totalReach = Advertisercampaigns.reduce(
    (acc, c) => acc + c.metrics.totalReach,
    0
  );
              ...
              <PopoverTrigger asChild>
                ...
                >
                  <div
                    ...
                    style={{
                      background: "linear-gradient(to right, #8637e9, #f6627e)",
                    }}
                  >
            ...
            <div className="">
              <div className="flex items-center gap-1">
                <CardTitle className="flex items-center justify-center gap-1">
                  {advertiser.fullName.split(" ")[0].charAt(0).toUpperCase() +
                    advertiser.fullName.split(" ")[0].slice(1)}
                  <Verified className="text-amber-500 h-[1em] w-[1em] relative top-[0.1em]" />
                </CardTitle>
              </div>
              <div>
                <CardDescription className="font-medium">
                  {advertiser.industry.charAt(0).toUpperCase() +
                    advertiser.industry.slice(1)}
                </CardDescription>
              </div>
            </div>
          </div>
